...
import { StaticSemanticGenerationPage } from "./pages/StaticSemanticGenerationPage";
...
const navItems: NavItem[] = [
  ...
  { id: 2, title: "语义注入页", shortTitle: "语义注入" },
  { id: 3, title: "静态语义生成页", shortTitle: "语义生成" },
  { id: 4, title: "低空世界总览", shortTitle: "世界总览" },
  { id: 5, title: "自然语言任务入口", shortTitle: "任务意图" },
  { id: 6, title: "资源调度页", shortTitle: "资源调度" },
  { id: 7, title: "航线规划与候选方案页", shortTitle: "航线规划" },
  { id: 8, title: "风险推演页", shortTitle: "风险推演" },
  { id: 9, title: "任务执行与动态重规划页", shortTitle: "执行重规划" },
  { id: 10, title: "结果交付与数据闭环页", shortTitle: "结果闭环" }
];
...
function getCompletedPage(state: DemoState): number {
  if (!state.activeTask) return 4;
  if (state.activeTask.status === "reported") return 10;
  if (state.activeTask.status === "completed") return 9;
  if (["executing", "event_detected", "replanned"].includes(state.activeTask.status)) return 8;
  if (state.activeTask.status === "risk_reviewed") return 8;
  if (state.activeTask.status === "planned") return 7;
  if (state.selectedDroneId && state.selectedDockId) return 6;
  if (state.activeTask.status === "parsed") return 5;
  return 4;
}
    ...
    <div className="app-shell">
      ...
      <div className="workspace">
        ...
        <main className="main-stage">
          ...
          {state.currentPage === 3 && <StaticSemanticGenerationPage goNext={goNext} />}
          {state.currentPage === 4 && <WorldOverviewPage state={state} goNext={goNext} />}
          {state.currentPage === 5 && <TaskIntentPage activeTask={state.activeTask} onTaskParsed={setParsedTask} goNext={goNext} />}
          {state.currentPage === 6 && (
            <ResourceDispatchPage state={state} onConfirmResource={confirmResourceSelection} goNext={goNext} goToTaskPage={() => setCurrentPage(5)} />
          )}
          {state.currentPage === 7 && (
            <RoutePlanningPage
              state={state}
              ...
              goNext={goNext}
              goToTaskPage={() => setCurrentPage(5)}
              goToResourcePage={() => setCurrentPage(6)}
            />
          )}
          {state.currentPage === 8 && (
            <RiskSimulationPage
              ...
              goNext={goNext}
              goToRoutePage={() => setCurrentPage(7)}
              goToTaskPage={() => setCurrentPage(5)}
            />
          )}
          {state.currentPage === 9 && <ExecutionReplanningPage state={state} onExecutionUpdate={updateExecutionState} goNext={goNext} goToRiskPage={() => setCurrentPage(8)} />}
          {state.currentPage === 10 && (
            <ResultDataLoopPage
              state={state}
              onReportGenerated={setMissionReport}
              goToExecutionPage={() => setCurrentPage(9)}
